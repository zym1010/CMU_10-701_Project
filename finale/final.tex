...
The system takes results generated from our classifier as input. For the results, our classifier needs to produce the belief of each sequence and the tagged device id with it. Then, the system produces value of area under the ROC curve (AUC) as the score. The baseline of AUC is 0.5 which stands for purely random guesses. The higher AUC value it is, the better classifier we have. A result with AUC of value 1 stands for perfect guess. 
...
Our preliminary results show that naive K-nearest-neighbor algorithm leads to really poor results which achieve less than 0.6 in AUC. Although there is an important parameter, number of neighbors, for KNN, we simply skipped the optimization of KNN for its poor performance and for its time-consuming computation.

We use logistic classifier as one of our main options. The only parameter can be tuned in this classifier is $C$ (in LIBLINEAR) as the regularization factor of logistic regression. It turns out that our features and data were not sensitive to the value of $C$ (no matter what combination of features used, AUC only fluctuated by about 0.0002). Therefore, we just used $C=1$.

We also tried support vector machine classifier. As it was very time-consuming, we only tested $C=1, g=1$ (LIBSVM) for a SVM with a Gaussian kernel.
