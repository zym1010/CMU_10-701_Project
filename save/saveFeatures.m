...
fft_quantile_overlap = zeros(length(train_label), 99+387*3);
...
%     fft_quantile_overlap(idx,100:104) = repmat(quantiles(i,3:7),length(idx),1);
    fft_quantile_overlap(idx,100:end) = repmat(overlap_strict_train(i,:),length(idx),1);
...
libsvmwrite('train_feature_2sets_sparse.txt',train_label,sparse(fft_quantile_overlap));
