...
fft_quantile_overlap_test = zeros(segment_num, 99+387*3+30);
...
%     fft_quantile_overlap_test(idx,100:104) = repmat(quantiles(i,3:7),length(idx),1);
    ...
    fft_quantile_overlap_test(idx,100+387*3:end) = repmat(feature_time_test(i,:),length(idx),1);

    disp(i);
end

libsvmwrite('test_feature_2sets_sparse_timehist.txt',test_label,sparse(fft_quantile_overlap_test));
