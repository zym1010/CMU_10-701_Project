...
load('unique_value_feature_test.mat');

segment_num = size(test_feature_all,1);

test_label = ones(segment_num,1);

fft_quantile_overlap_test = zeros(segment_num, 99+387*3+546);
...
%     fft_quantile_overlap_test(idx,100:104) = repmat(quantiles(i,3:7),length(idx),1);
    ...
    fft_quantile_overlap_test(idx,100+387*3:end) = repmat(unique_value_feature_test(i,:),length(idx),1);

    disp(i);
end

libsvmwrite('test_feature_2sets_sparse_unique.txt',test_label,sparse(fft_quantile_overlap_test));
