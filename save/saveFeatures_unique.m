...
load('unique_value_feature_train.mat');
% load('quantiles.mat');


fft_quantile_overlap = zeros(length(train_label), 99+387*3+546);
...
%     fft_quantile_overlap(idx,100:104) = repmat(quantiles(i,3:7),length(idx),1);
    ...
    fft_quantile_overlap(idx,100+387*3:end) = repmat(unique_value_feature_train(i,:),length(idx),1);
...
libsvmwrite('train_feature_2sets_sparse_unique.txt',train_label,sparse(fft_quantile_overlap));
